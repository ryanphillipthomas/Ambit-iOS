...
public struct Utilities {
    ...
    public static func colorFromXY(xy: CGPoint, forModel model: String) -> SwiftyHueColor {
        ...
        var xy = xy
        let colorPoints: [NSValue] = colorPointsForModel(model: model)
        let inReachOfLamps: Bool = checkPointInLampsReach(p: xy, withColorPoints: colorPoints)
        ...
        if !inReachOfLamps {
            ...
            let pAB: CGPoint = getClosestPointToPoints(point1: getPointFromValue(point: colorPoints[cptRED]), point2: getPointFromValue(point: colorPoints[cptGREEN]), point3: xy)
            let pAC: CGPoint = getClosestPointToPoints(point1: getPointFromValue(point: colorPoints[cptBLUE]), point2: getPointFromValue(point: colorPoints[cptRED]), point3: xy)
            let pBC: CGPoint = getClosestPointToPoints(point1: getPointFromValue(point: colorPoints[cptGREEN]), point2: getPointFromValue(point: colorPoints[cptBLUE]), point3: xy)
     ...
     */
    public static func calculateXY(cgColor: CGColor, forModel model: String) -> CGPoint {
        ...
        let colorPoints: [NSValue] = colorPointsForModel(model: model)
        let inReachOfLamps: Bool = checkPointInLampsReach(p: xyPoint, withColorPoints: colorPoints)
        ...
        if !inReachOfLamps {
            ...
            //Find the closest point on each line in the triangle.
            let pAB = getClosestPointToPoints(point1: getPointFromValue(point: colorPoints[cptRED]), point2: getPointFromValue(point: colorPoints[cptGREEN]), point3: xyPoint)
            let pAC = getClosestPointToPoints(point1: getPointFromValue(point: colorPoints[cptBLUE]), point2: getPointFromValue(point: colorPoints[cptRED]), point3: xyPoint)
            let pBC = getClosestPointToPoints(point1: getPointFromValue(point: colorPoints[cptGREEN]), point2: getPointFromValue(point: colorPoints[cptBLUE]), point3: xyPoint)
    ...
    private static func colorPointsForModel(model: String) -> [NSValue] {
        ...
        if gamutA.contains(model) {
            ...
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.703, y: 0.296)))  // Red
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.214, y: 0.709)))  // Green
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.139, y: 0.081)))  // Blue
            
        } else if gamutB.contains(model) {
            
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.674, y: 0.322)))  // Red
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.408, y: 0.517)))  // Green
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.168, y: 0.041)))  // Blue
            
        } else if gamutC.contains(model) {
            
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.692, y: 0.308)))  // Red
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.17, y: 0.7)))     // Green
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.153, y: 0.048)))  // Blue
            
        } else {
            
            // Default construct triangle wich contains all values
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 1.0, y: 0.0)))      // Red
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.0, y: 1.0)))      // Green
            colorPoints.append(getValueFromPoint(point: CGPoint(x: 0.0, y: 0.0)))      // Blue
        }
    ...
    private static func checkPointInLampsReach(p: CGPoint, withColorPoints colorPoints:[NSValue]) -> Bool {
        ...
        let red: CGPoint =   getPointFromValue(point: colorPoints[cptRED])
        let green: CGPoint = getPointFromValue(point: colorPoints[cptGREEN])
        let blue: CGPoint =  getPointFromValue(point: colorPoints[cptBLUE])
